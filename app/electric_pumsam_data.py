...
ELECTRIC_RULES = [
    ...
    "14. 명칭이 같고 규격이 글자 그대로 같으면 그대로 붙습니다. 딱 맞는 규격이 없고 「N㎟ 이하」처럼 이하 구간만 있으면, 품목 규격 이상인 가장 작은 이하 구간을 씁니다. 예: 4㎟ → 6㎟ 이하, 3㎟ → 3㎟ 이하. 지중/노출, 1C/3C처럼 다른 말은 끌어오지 않습니다.",
    "15. 품셈 칸은 표준품셈 원표 숫자입니다. 할증을 곱하지 않은 값입니다. 할증은 할증% 칸에 따로 적고, 일위대가 인부 규격·비고와 수량 수식(품셈×할증%)으로 보여 줍니다.",
